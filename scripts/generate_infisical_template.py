...
3. Start the container. The wrapper will auto-generate and persist [code]ENCRYPTION_KEY[/code] and [code]AUTH_SECRET[/code] if you leave them blank, and bundled Mailpit inbox UI credentials will be stored in [code]/config/aio/generated.env[/code]. Publish [code]Local Mail Inbox Port[/code] only when you intentionally need host-network inbox access.&#xD;
...
- If you enable automatic bootstrap, you are creating a highly privileged instance-admin identity during first boot. Treat those credentials carefully.</Overview>
  ...
  <Config Name="Local Mail Inbox Port" Target="8025" Default="" Mode="tcp" Description="Optional host port for the bundled Mailpit inbox UI used in local or lab email capture. Leave blank unless you intentionally need inbox UI access from the host network." Type="Port" Display="advanced" Required="false" Mask="false"></Config>
